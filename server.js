...
app.get("/api/vinted/download-txt", async (req, res) => {
  try {
    ...
    if (!row) return res.status(404).send("Articolo non trovato nello sheet");
    ...
    // 3. Genera TXT per download locale
...
app.get("/dashboard", (req, res) => {
  ...
  <body>
    ...
    <div id="status"></div>
    <table>
      <thead><tr><th>Codice</th><th>Descrizione & Mancanti</th><th>Stato</th><th>Azione</th></tr></thead>
      ...
      async function load(){
        ...
        tb.innerHTML = j.articoli.map(a => {
          ...
          return \`<tr>
            ...
            <td><button onclick="process('\${a.codice}')">Crea Drive + TXT</button></td>
          </tr>\`;
        }).join('');
      }
      function process(c){ window.location.href = '/api/vinted/download-txt?codice=' + c; }
      load();
    </script></body></html>`);
});
...
/* =========================================================
   ...
   ========================================================= */
app.listen(PORT, () => {
  console.log(`🚀 SyncFED avviato sulla porta ${PORT} - File aggiornato con Google Drive Integration`);
});

// Fine del file server.js
